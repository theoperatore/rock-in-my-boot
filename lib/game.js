...
const log = require('debug')('rockin:messages');
const {
  CLIENT_CONNECTED,
  CREATE_GROUP,
  JOIN_GROUP,
...
module.exports = function initializeGame(emit) {
  ...
  return function createMessageHandler(uid) {
    return function handleMessage(action) {
      log('[ %s ] action: %o', uid, action);
      switch (action.type) {
        case 'CURRENT_STATE':
          handleSendCurrentState(uid, action);
          break;
